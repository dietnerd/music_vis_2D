...
void Shape::draw(const shared_ptr<Program> prog, bool use_extern_texures)
{
	for (int i = 0; i < obj_count; i++)
	{
        static int h_pos, h_nor, h_tex;
        h_pos = h_nor = h_tex = -1;
		...
		// Draw
		glDrawElementsInstanced(GL_TRIANGLES, (int)eleBuf[i].size(), GL_UNSIGNED_INT, (const void *)0, TEXSIZE * TEXSIZE);
